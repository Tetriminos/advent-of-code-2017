let instructionsRaw = `ba dec 37 if znx != 0
...
let instructions = instructionsRaw.split('\n');
let prettyInstructions = [];

for (let i = 0; i < instructions.length; i++) {
    currentInstructionArray = instructions[i].split(' ');
    let currentInstructionObject = {
        register: currentInstructionArray[0],
        action: currentInstructionArray[1],
        value: currentInstructionArray[2],
        comparationRegister: currentInstructionArray[4],
        comparation: currentInstructionArray[5],
        comparationValue: currentInstructionArray[6],
    }
    prettyInstructions.push(currentInstructionObject);
}

let registers = {}

for (let i = 0; i < instructions.length; i++) {
    if (registers[prettyInstructions[i].register] === undefined) {
        registers[prettyInstructions[i].register] = 0;
    }

    if (registers[prettyInstructions[i].comparationRegister] === undefined) {
        registers[prettyInstructions[i].comparationRegister] = 0;
    }
    switch(prettyInstructions[i].comparation) {
        case '>':
            if (greaterThan(registers[prettyInstructions[i].comparationRegister], prettyInstructions[i].comparationValue)) {
                switch(prettyInstructions[i].action) {
                    case 'inc':
                        registers[prettyInstructions[i].register] = increment(registers[prettyInstructions[i].register], prettyInstructions[i].value);
                        break;
                    case 'dec':
                        registers[prettyInstructions[i].register] = decrement(registers[prettyInstructions[i].register], prettyInstructions[i].value);
                        break;
                }
            }
            break;
        case '<':
            if (lesserThan(registers[prettyInstructions[i].comparationRegister], prettyInstructions[i].comparationValue)) {
                switch(prettyInstructions[i].action) {
                    case 'inc':
                        registers[prettyInstructions[i].register] = increment(registers[prettyInstructions[i].register], prettyInstructions[i].value);
                        break;
            ...
            break;
        case '==':
            if (equal(registers[prettyInstructions[i].comparationRegister], prettyInstructions[i].comparationValue)) {
                switch(prettyInstructions[i].action) {
                    case 'inc':
                        registers[prettyInstructions[i].register] = increment(registers[prettyInstructions[i].register], prettyInstructions[i].value);
                        break;
            ...
            break;
        case '!=':
            if (notEqual(registers[prettyInstructions[i].comparationRegister], prettyInstructions[i].comparationValue)) {
                switch(prettyInstructions[i].action) {
                    case 'inc':
                        registers[prettyInstructions[i].register] = increment(registers[prettyInstructions[i].register], prettyInstructions[i].value);
                        break;
            ...
            break;
        case '>=':
            if (greaterThanOrEqual(registers[prettyInstructions[i].comparationRegister], prettyInstructions[i].comparationValue)) {
                switch(prettyInstructions[i].action) {
                    case 'inc':
                        registers[prettyInstructions[i].register] = increment(registers[prettyInstructions[i].register], prettyInstructions[i].value);
                        break;
            ...
            break;
        case '<=':
            if (lesserThanOrEqual(registers[prettyInstructions[i].comparationRegister], prettyInstructions[i].comparationValue)) {
                switch(prettyInstructions[i].action) {
                    case 'inc':
                        registers[prettyInstructions[i].register] = increment(registers[prettyInstructions[i].register], prettyInstructions[i].value);
                        break;
    ...
    for (let j in registers) {
        console.log(`${j}: ${registers[j]}`);
    }
    console.log('======');
}
...
function decrement(register, value) {
    return register - parseInt(value);
}

function increment(register, value) {
    return register + parseInt(value);
}

function greaterThan(comparationRegister, comparationValue) {
    if (comparationRegister > parseInt(comparationValue)) {
        return true;
    } else {
        return false;
    }
...
function lesserThan(comparationRegister, comparationValue) {
    if (comparationRegister < parseInt(comparationValue)) {
        return true;
    } else {
        return false;
...
function equal(comparationRegister, comparationValue) {
    if (comparationRegister === parseInt(comparationValue)) {
        return true;
    } else {
        return false;
...
function notEqual(comparationRegister, comparationValue) {
    if (comparationRegister !== parseInt(comparationValue)) {
        return true;
    } else {
        return false;
...
function greaterThanOrEqual(comparationRegister, comparationValue) {
    if (comparationRegister >= parseInt(comparationValue)) {
        return true;
    } else {
        return false;
...
function lesserThanOrEqual(comparationRegister, comparationValue) {
    if (comparationRegister <= parseInt(comparationValue)) {
        return true;
    } else {
        return false;
