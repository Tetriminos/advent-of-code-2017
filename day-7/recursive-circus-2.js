...
let discs = discList.split('\n');
let childlessDiscs = [];
let parentDiscs = {};
// this one won't have its contents modified
let parentDiscsPersistent = {};
let discWeightMap = {};

console.log(discs);

for (let i = 0; i < discs.length; i++) {
    if (discs[i].indexOf('-> ') === -1) {
        childlessDiscs.push(discs[i].substring(0, discs[i].indexOf(' ')));
    } else {
        let childrenString = discs[i].substring(discs[i].indexOf('-> ') + 3);
        let childrenArray = childrenString.split(', ');
        parentDiscs[discs[i].substring(0, discs[i].indexOf(' '))] = { childrenArray: childrenArray };
        parentDiscsPersistent[discs[i].substring(0, discs[i].indexOf(' '))] = { childrenArray: childrenArray.slice() };
    }

    discWeightMap[discs[i].substring(0, discs[i].indexOf(' '))] = parseInt(discs[i].substring(discs[i].indexOf('(') + 1, discs[i].indexOf(')')));
}

// console.log(childlessDiscs);
// console.log(JSON.stringify(parentDiscs, undefined, 4));
let parentDepth = {};
let depthCounter = 0;

function findAllParents(childlessDiscs) {
    let newLayer = [];
    for (let i = 0; i < childlessDiscs.length; i++) {
        for (let j in parentDiscs) {
            if (parentDiscs[j].childrenArray.includes(childlessDiscs[i])) {
                parentDiscs[j].childrenArray.splice(parentDiscs[j].childrenArray.indexOf(childlessDiscs[i]), 1);
                if (parentDiscs[j].childrenArray.length === 0) {
                    newLayer.push(j);
                    delete parentDiscs[j];
                }
                break;
            }
        }
    ...
    if (Object.keys(parentDiscs).length > 0) {
        parentDepth[depthCounter] = newLayer;
        depthCounter++;
        findAllParents(newLayer);
    } else {
        parentDepth[depthCounter] = newLayer;
        depthCounter++;
    }
...
findAllParents(childlessDiscs);

for (let i = 0; i < depthCounter; i++) {
    for (let j = 0; j < parentDepth[i].length; j++) {
        let currentDiscWeightSum = 0;
        let currentValue = discWeightMap[parentDiscsPersistent[parentDepth[i][j]].childrenArray[0]];
        let countOfWrong = 0;
        for (let k = 0; k < parentDiscsPersistent[parentDepth[i][j]].childrenArray.length; k++) {
            if (currentValue !== discWeightMap[parentDiscsPersistent[parentDepth[i][j]].childrenArray[k]]) {
                countOfWrong++;
            }
            currentDiscWeightSum += discWeightMap[parentDiscsPersistent[parentDepth[i][j]].childrenArray[k]];
        }

        if (countOfWrong > 1) {
            console.log('the difference in the fucky one (' + parentDiscsPersistent[parentDepth[i][j]].childrenArray[0] + ') is ' + (currentValue - discWeightMap[parentDiscsPersistent[parentDepth[i][j]].childrenArray[2]]));
            break;
        } else if (countOfWrong === 1) {
            for (let k = 0; k < parentDiscsPersistent[parentDepth[i][j]].childrenArray.length; k++) {
                if (currentValue !== discWeightMap[parentDiscsPersistent[parentDepth[i][j]].childrenArray[k]]) {
                    console.log('the difference in the fucky one (' + parentDiscsPersistent[parentDepth[i][j]].childrenArray[k] + ') is ' + (currentValue - discWeightMap[parentDiscsPersistent[parentDepth[i][j]].childrenArray[k]]));
                    break;
                }
            }
            break;
        ...
        discWeightMap[parentDepth[i][j]] += currentDiscWeightSum;

        console.log(`For disc [${parentDepth[i][j]}], which is at depth [${i}], the combined weight is ${discWeightMap[parentDepth[i][j]]}`);
    }


}

// console.log(JSON.stringify(discWeightMap, undefined, 4));
