...
let isPassphraseValid = (string, wordArray) => {
    let matchArray = null;
    for (let i = 0; i < wordArray.length; i++) {
        matchArray = string.match(new RegExp('\\b' + wordArray[i] + '\\b', 'g'));
...
let inputInRows = splitRows(input);

let validCount = 0;
let invalidCount = 0;

for (let i = 0; i < inputInRows.length; i++) {
    if (isPassphraseValid(inputInRows[i], splitIntoWords(inputInRows[i])) === true) {
        validCount++;
    } else {
        invalidCount++;
    }
...
console.log(`validCount: ${validCount}, invalidCount: ${invalidCount}`);
